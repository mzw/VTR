...
import org.eclipse.jdt.internal.corext.fix.JdtASTMatcher;
...
public class ConvertForLoopOperation extends ConvertLoopOperation {
	...
	public String getIntroducedVariableName() {
		if (fElementDeclaration != null) {
			...
			String[] proposals= getVariableNameProposals(fArrayAccess.resolveTypeBinding(), null);
			return proposals[0];
		}
	}
			...
			private void replaceAccess(ASTNode node) {
				if (fElementDeclaration != null && node.getLocationInParent() == VariableDeclarationFragment.INITIALIZER_PROPERTY) {
					...
					if (targetBinding != null) {
						...
						System.out.println("replace " + node + " with " + name);
	...
	@Override
	public EnhancedForStatement convert(String source, CompilationUnit cu) throws CoreException {
		ForStatement forStatement= getForStatement();
		...
		LinkedProposalPositionGroup pg = new LinkedProposalModel().getPositionGroup(parameterName, true);
		if (fElementDeclaration != null)
			pg.addProposal(parameterName, null, 10);
		for (int i= 0; i < proposals.length; i++) {
			pg.addProposal(proposals[i], null, 10);
		...
		AST ast = cu.getAST();
		SingleVariableDeclaration parameter = createParameterDeclaration(parameterName, fElementDeclaration, fArrayAccess, forStatement, null, rewrite, null, pg, fMakeFinal);
		Expression expression = ast.newSimpleName(fArrayAccess.toString());

		List<ConvertInfo> converts = getConvertInfo(forStatement.getBody(), fIndexBinding, fArrayBinding, parameterName, rewrite, null, pg);
		char[] sourceCharArray = source.toCharArray();
		StringBuilder builder = new StringBuilder();
		for (int pos = 0; pos < sourceCharArray.length; pos++) {
			boolean target = false;
			if (forStatement.getStartPosition() <= pos && pos < forStatement.getBody().getStartPosition()) {
				target = true;
				builder.append("for (").append(parameter).append(" : ").append(expression).append(") ");
				for (int length = 0; length < forStatement.getBody().getStartPosition() - forStatement.getStartPosition() - 1; length++) {
					pos++;
				}
			}
			for (ConvertInfo convert : converts) {
				if (convert.getStartPosition() <= pos && pos < convert.getStartPosition() + convert.getLength()) {
					target = true;
					builder.append(convert.getReplace());
					for (int length = 0; length < convert.getLength() - 1; length++) {
						pos++;
					}
					break;
				}
			...
			if (!target) {
				builder.append(sourceCharArray[pos]);
			}
		}
		
		final List<EnhancedForStatement> targets = new ArrayList<>();
		...
		compilationUnit.accept(new GenericVisitor() {
			@Override
		...
		for (EnhancedForStatement target : targets) {
			if (target.getStartPosition() == forStatement.getStartPosition()) {
				return target;
			}
		}
	...
	private List<ConvertInfo> getConvertInfo(Statement body, final IBinding indexBinding, final IBinding arrayBinding, final String parameterName, final ASTRewrite rewrite, final TextEditGroup editGroup, final LinkedProposalPositionGroup pg) {
		final List<ConvertInfo> ret = new ArrayList<>();
			...
			private void replaceAccess(ASTNode node) {
				if (fElementDeclaration != null && node.getLocationInParent() == VariableDeclarationFragment.INITIALIZER_PROPERTY) {
					...
					if (targetBinding != null) {
						...
						if (statement.fragments().size() == 1) {
							rewrite.remove(statement, editGroup);							
							ret.add(new ConvertInfo(statement.getStartPosition(), statement.getLength(), ""));
						} else {
							ListRewrite listRewrite= rewrite.getListRewrite(statement, VariableDeclarationStatement.FRAGMENTS_PROPERTY);
							listRewrite.remove(fragment, editGroup);
							ret.add(new ConvertInfo(fragment.getStartPosition(), fragment.getLength(), ""));
						}
						...
						rewrite.replace(node, name, editGroup);
						pg.addPosition(rewrite.track(name), true);
						ret.add(new ConvertInfo(node.getStartPosition(), node.getLength(), parameterName));
					}
				} else {
					SimpleName name= ast.newSimpleName(parameterName);
					rewrite.replace(node, name, editGroup);
	...
	public static class ConvertInfo {
		...
		public ConvertInfo(int start, int length, String replace) {
			this.start = start;
			this.length = length;
	...
	private SingleVariableDeclaration createParameterDeclaration(String parameterName, VariableDeclarationFragment fragement, Expression arrayAccess, ForStatement statement, ImportRewrite importRewrite, ASTRewrite rewrite, TextEditGroup group, LinkedProposalPositionGroup pg, boolean makeFinal) {
		...
		if (importRewrite != null) {
			type = importType(arrayTypeBinding.getElementType(), statement, importRewrite, compilationUnit);
		} else {
